...
for u in range(num_unrollings):
    train_inp.append(tf.placeholder(tf.float32, shape=[batch_size,1],name='train_inputs_%d'%u))
    train_out.append(tf.placeholder(tf.float32, shape=[batch_size,1], name = 'train_outputs_%d'%u))
...
with tf.control_dependencies([tf.assign(c[li], state[li][0]) for li in range(n_layers)]+
  ...
  for ui in range(num_unrollings):
    loss += tf.reduce_mean(0.5*(split_outputs[ui]-train_out[ui])**2)
...
sample_inputs = tf.placeholder(tf.float32, shape=[1,1])
